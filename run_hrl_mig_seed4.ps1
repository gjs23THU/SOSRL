...
$schedulerCheckpoint = Join-Path $repoRoot "runs\MIG1000\model.pt"
$runRoot = Join-Path $repoRoot "runs\hrl_budget20_MIG_seed4"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "MIG architecture training exited with code $LASTEXITCODE."
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "MIG evaluation exited with code $LASTEXITCODE."
    }
